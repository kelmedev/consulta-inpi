  ...
  <div className="p-4 md:p-5">
    <div className="flex flex-col gap-4 text-center">
      ...
      <p className="text-white font-medium text-sm md:text-base lg:text-lg whitespace-normal">
        ...
        para visualizar. Digite seu e-mail abaixo para que possamos enviar também por lá.
      </p>
      ...
      <form onSubmit={formik.handleSubmit} className="flex flex-col gap-4 mt-2">
        <div className="flex flex-col gap-2">
          ...
          {formik.touched.name && formik.errors.name ? (
            <span className="text-start text-red-500 text-sm">{formik.errors.name}</span>
          ) : null}
        </div>
        <div className="flex flex-col gap-2">
          ...
          {formik.touched.email && formik.errors.email ? (
            <span className="text-start text-red-500 text-sm">{formik.errors.email}</span>
          ) : null}
        </div>
        <DialogFooter className="flex flex-col w-full mt-2">
          {loading ? (
            <div className="space-y-2 w-full">
              ...
              <p className="text-blue-400 text-sm text-center">{progress}%</p>
            </div>
          ) : (
            <Button
              type="submit"
    ...
    <div className="relative w-full border-2 border-slate-700 bg-slate-800 rounded-lg overflow-hidden">
      <div className="p-3 md:p-4 space-y-3">
        ...
        <p className="text-slate-200 text-sm md:text-base">
          <span className="text-red-500 font-bold">🚀 PRESENTE ESPECIAL: </span>
          Descubra se sua marca está dentro das diretrizes de registro! Com nosso
          <span className="font-bold"> Relatório Profissional</span>, você recebe uma análise completa
          de viabilidade e brand-fit para evitar problemas no futuro.
        </p>
        <Button className="w-full bg-green-600 hover:bg-green-700 text-white font-bold py-2 md:py-3 h-10 md:h-12 rounded-lg flex items-center justify-center gap-2">
          ...
          <span className="whitespace-nowrap text-sm md:text-base">Desbloquear Agora por Apenas R$ 29,90!</span>
        </Button>
      </div>
    </div>
  </div>
